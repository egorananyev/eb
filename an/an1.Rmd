...
lmerfn(lmer(RT~Cond*cue_valid*cue_pred + (1|subj), data=ds_order))
```
...
lmerfn(lmer(RT ~ Cond * cue_valid * cue_pred + sess_order_0 + cond_order_c + block_order_c + 
...
lmerfn(lmer(RT ~ blink_dur_c * cue_valid * cue_pred * Cond + (1|subj),  data=dsa_temp))
```
...
lmerfn(lmer(RT ~ blink_dur_c * Cond * cue_valid * cue_pred + sess_order_0 + cond_order_c + block_order_c + 
...
Looking into the effects of nuisance parameters on blink duration:
```{r}
lmerfn(lmer(blink_dur ~ Cond * sess_order_0 * cond_order_c * block_order_c + (1|subj), data=dsa_order))
```
