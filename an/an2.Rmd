...
cloud_dir = 'C:/gd/'
if(computer == 'shocky' || computer == 'toshi'){ prog_dir = 'C:/Users/Egor/Prog/' }
...
ds = read.csv(paste0(prog_dir, 'Projects/eb/eb/an/consolidated/ds2.csv'))
dsa = read.csv(paste0(prog_dir, 'Projects/eb/eb/an/consolidated/dsa2.csv'))
...
## Heavy smoothing (gam)
p = ggplot(data=ds, aes(x=ctoa, y=RT, linetype=CueValid, color=Cond)) + 
    geom_smooth(method='gam', alpha=0.15, aes(fill=Cond), formula=y~s(x, bs="cs")) +
    xlab('Cue-to-Target Asynchrony (ms)') + ylab('RT (ms)') + facet_grid(.~Cond) + guides(alpha=F) +
    theme(panel.grid.minor=element_blank(), plot.title = element_text(hjust = 0.5))
p = plot_themefy(p) + theme(legend.position='none')
if(out){outpdf('eb2_rt~ctoa_gam', w=8, h=2)}
plot(p)
