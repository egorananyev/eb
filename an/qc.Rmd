...
if(lab){dropbox_dir='/home/station3/Desktop/Egor/Dropbox/'}
library(R.utils)
sourceDirectory(paste(dropbox_dir, 'Projects/eb/eb/an/funs', sep='/'))
# Getting the data directory:
data_dir = get_dir(subj, cond, sess, dropbox_dir)
# Reading raw data:
...
if(cond == 'cond-c' | cond == 'cond-a'){
    if(cond == 'cond-a'){
        ...
        artif_inds = with(merged, blank_sample_beg >= shutter_sample_beg &
                                  blank_sample_end <= shutter_sample_end + 10)  # so, ...
    ...
    excl_prohib_blink_trials = with(merged, blank_time_beg > (cue_time - trial_time_beg) &
                               blank_time_beg < (targ_time - trial_time_beg) & nonart)
