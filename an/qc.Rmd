...
# In case there are no blinks, we need to have a convenient trigger: blink_nada
# if(nrow(blanks) == 1 & is.na(blanks[1,1])){ blink_nada = TRUE } else { blink_nada = FALSE }
```
...
if(sum(trials$sacc_bef_targ, na.rm = T)){
  excl_sacc_trials = data.frame(trial=trials$trial[trials$sacc_bef_targ == 1 &
                                                   !is.na(trials$sacc_bef_targ)],
                                excl_reason = 'saccade')
} else {
  excl_sacc_trials = NULL
}
```
...
all_excl = rbind(excl_targ_blink, excl_prohib_blink, excl_no_blink, excl_sacc_trials)
