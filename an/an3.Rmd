...
if(computer=='acer' || computer == 'toshi'){ cloud_dir = 'Users/egora/Dropbox/' }
...
## Heavy smoothing (gam)
p = ggplot(data=ds, aes(x=targ_soa, y=RT, color=CueValid)) + 
...
p = plot_themefy(p)
if(out){outpdf('eb3_rt~ctoa_gam', w=5.5, h=3.5)}
plot(p)
...
p = ggplot(data=ds, aes(x=targ_soa, y=RT, color=CueValid)) + 
    ...
    theme(panel.grid.minor=element_blank(), plot.title = element_text(hjust = 0.5))
p = plot_themefy(p)
if(out){outpdf('eb3_rt~ctoa_loess', w=5.5, h=3.5)}
plot(p)
```
...
p = plot_themefy(p)
if(out){outpdf('eb3_adv-spectra~ctoa', h=1.7, s=T)}
...
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-z-split&ctoa-disc_box', w=7, h=3.5)}
print(p)
```
...
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-z-split&ctoa-cont_curve', w=5.5)}
print(p)
p = ggplot(dsa, aes(x=ctoa, y=RT, color=CueValid)) + 
    ...
    xlab('CTOA (ms)') + ylab('Reaction Time (ms)')
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-z-split&ctoa-cont_lin', w=5.5)}
print(p)
```
...
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-z&ctoa-disc_lin', w=5.5, h=3.5, s=T)}
suppressWarnings(plot(p))
```
...
p = ggplot(dsa, aes(x=blink_dur_z, y=RT, color=CueValid)) + 
    ...
    # xlim(-2.5, 2.5)
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-z&ctoa-disc_lin_full-range', w=5.5, h=2.5, s=T)}
suppressWarnings(plot(p))
```
...
if(out){outpdf('eb3_rt~bdur-z&ctoa-disc_slopes-lin', w=5.5, h=4.5, s=T)}
suppressWarnings(plot(p))
...
p = ggplot(dsa, aes(x=blink_dur_z, y=RT, lty=CueValid)) + 
    ...
    guides(color=F) + theme_bw()
# p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-z&ctoa-disc_fit', w=5.5, h=4.5, s=T)}
suppressWarnings(plot(p))
```
...
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-z&ctoa-disc_slopes-cor', w=7.0, h=3.5, s=T)}
print(p)
```
...
p = ggplot(bdur_rt_cond, aes(x=bdur, y=RT)) + geom_point(aes(color=factor(CueValid))) + 
    ...
    theme(legend.position=c(.09, .8), legend.title=element_blank())
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-indiv_lin', w=5.5)}
print(p)
```
...
p = ggplot(bdur_rt_cond_ctoa, aes(x=bdur, y=RT)) + geom_point(aes(color=factor(CueValid))) + 
    ...
    guides(colour=guide_legend(title='Cue')) + theme_bw()
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-indiv&ctoa-disc_lin', w=5.5)}
plot(p)
```
...
p = ggplot(dsa, aes(x=blink_dur_ms, y=RT, color=CueValid)) + 
    ...
    xlab('Blink Duration (ms)') + ylab('Reaction Time (ms)')
p = plot_themefy(p)
if(out){outpdf('eb3_rt~bdur-raw&ctoa-disc_lin', w=5.5, h=3.5, s=T)}
print(p)
```
...
p = plot_themefy(p)
if(out){outpdf('eb3_rt~blat-indiv_ctoa', w=6.5, h=2.5, s=T)}
plot(p)
```
...
p = ggplot(dsa, aes(x=blat_z, y=RT, color=CueValid)) + 
    stat_smooth(method='lm', alpha=0.15) + facet_grid(Cond~ctoa3) + xlim(-2.5, 2.5)
p = plot_themefy(p)
if(out){outpdf('eb3_rt~blat-z&ctoa-disc_lin', w=5.5, h=3.5, s=T)}
suppressWarnings(plot(p))
```
...
p = ggplot(btoa_rt, aes(x=btoa, y=RT)) + geom_point(aes(color=CueValid)) + 
  ...
  theme(legend.position=c(1.1, .75), legend.title=element_blank(), 
        ...
        legend.spacing.y = unit(0, "mm"))
p = plot_themefy(p)
if(out){outpdf('eb3_rt~btoa-indiv_ctoa', w=6.5, h=2.5, s=T)}
plot(p)
```
...
p = ggplot(dsa, aes(x=btoa_z, y=RT, color=CueValid)) + 
    stat_smooth(method='lm', alpha=0.15) + facet_grid(Cond~ctoa3) + xlim(-2.5, 2.5)
p = plot_themefy(p)
if(out){outpdf('eb3_rt~btoa-z&ctoa-disc_lin', w=5.5, h=3.5, s=T)}
suppressWarnings(plot(p))
```
