...
for(cur_cue_cond in cue_conds){
  ...
  for(cur_subj in subjs){
    ...
    for(cur_cond in conds){
      ## Keeping track of the block number:
      block_order = 0
...
colnames(df)[5] = 'trial'  # renaming the 'trial_id' column to 'trial'
head(df)
ds = df
# print(excl)
```
...
# Data output
```{r}
write.csv(ds, 'ds1.csv', row.names=F)
write.csv(dsa, 'dsa1.csv', row.names=F)
```