...
p = ggplot(data=ds_temp, aes(x=ctoa, y=RT, linetype=CueValid, color=Cond)) + 
...
p = p + guides(linetype=guide_legend(title=cue_valid_name, 
                                                   override.aes=list(col='black')),
                             color=F, alpha=F, fill=F)
p = plot_themefy(p)
# + theme(legend.position='none')
if(out){outpdf('eb2_rt~ctoa_gam', w=8, h=3)}
plot(p)
```
