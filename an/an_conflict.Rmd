...
ggplot(bdur_rt, aes(x=bdur, y=RT)) + geom_point(aes(color=factor(cue_valid_full))) + 
  ...
  theme(legend.position=c(.08, .75), legend.title=element_blank())
## Linear model:
...
ggplot(blat_rt, aes(x=blat, y=RT)) + geom_point(aes(color=factor(cue_valid_full))) + 
  ...
  guides(colour=guide_legend(title=cue_valid_name)) + 
  theme(legend.position=c(.08, .75), legend.title=element_blank()) + theme_bw()
## Linear model:
